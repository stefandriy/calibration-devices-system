angular
    ...
    .controller('OrganizationModalAddressController', ['$rootScope', '$scope', '$modalInstance', 'AddressService',
        function ($rootScope, $scope, $modalInstance, addressService) {
            ...
            /**
             * Init form data from $rootScope and loads needed dependency
             */
            function initFormData() {
                if (!$rootScope.address.region) {
                    ...
                    $scope.onRegionSelected($rootScope.address.region.id);
                } else if (!$rootScope.address.locality) {
                    $scope.onDistrictSelected($rootScope.address.district.id);
                } else if (!$rootScope.address.street) {
                    $scope.onLocalitySelected($rootScope.address.locality.id);
                } else if (!$rootScope.address.building) {
                    $scope.onStreetSelected($rootScope.address.street.id);
                }
            }
