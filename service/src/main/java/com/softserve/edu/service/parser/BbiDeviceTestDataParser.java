...
public class BbiDeviceTestDataParser implements DeviceTestDataParser {
    ...
    public DeviceTestData parse(InputStream deviceTestDataStream) {
        ...
        try {
            ...
            resultMap.put("unixTime", readLongValueReversed(4)); //0x800008 + 0x04
            ...
            resultMap.put("latitude", readLongValueReversed(4));  //0x800054+0x04
            resultMap.put("longitude", readLongValueReversed(4)); //0x800058+0x04
            ...
            resultMap.put("testPhoto", readImageHex());
            for (int i = 0; i < 12; ++i) {
                String imageKey = "test" + (i / 2 + 1) + (i % 2 == 0 ? "begin" : "end") + "Photo";
                resultMap.put(imageKey, readImageHex());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void readTest(int testIndex) throws IOException {
        ...
        resultMap.put("test" + testIndex + "allowableError", readLongValueReversed(4)); //0x80010с+0x04
        resultMap.put("test" + testIndex + "specifiedImpulsesAmount", readLongValueReversed(4)); //0x800110+0x04
        ...
        resultMap.put("test" + testIndex + "initialCounterValue", readLongValueReversed(4)); //0x800128+0x04
        resultMap.put("test" + testIndex + "terminalCounterValue", readLongValueReversed(4)); //0x80012с+0x04
        ...
        resultMap.put("test" + testIndex + "testDuration", readLongValueReversed(4)); //0x800138+0x04
     ...
     * @return Image written in hex string.
     * @throws IOException
     */
    private String readImageHex() throws IOException {
